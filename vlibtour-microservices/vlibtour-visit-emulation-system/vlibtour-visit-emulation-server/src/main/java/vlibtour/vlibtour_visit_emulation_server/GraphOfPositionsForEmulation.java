...
public final class GraphOfPositionsForEmulation {
	...
	/**
	 * creates an edge by considering the departure and the destination positions.
	 * <p>
	...
	Map<Position, Set<Position>> addEdge(final Map<Position, Set<Position>> adjacencySets,
		...
		Set<Position> neighbours = result.computeIfAbsent(departure, k -> new HashSet<>());
	...
	private <T> List<List<T>> depthFirstSearchRecursive(final Map<T, Set<T>> adjacencySets,
		...
		if (departure.equals(destination)) {
			...
			for (T w : adjacencySets.getOrDefault(departure, Collections.emptySet())) {
				if (Boolean.FALSE.equals(visited.getOrDefault(w, false))) {
	...
	/**
	 * gets the possible paths to the next positions, chose one of the paths, and set
